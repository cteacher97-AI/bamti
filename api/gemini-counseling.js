...
export default async function handler(req, res) {
  if (req.method !== 'POST') {
    return res.status(405).json({ success: false, error: 'Method Not Allowed. POST만 허용됩니다.' });
  }
...
아래 제공된 학생의 정보를 바탕으로 교사가 학생과 상담할 때 참고할 수 있는 전략을 제안해주세요.
...
2. “의지가 부족하다”, “주의력 문제가 있다”, “심리적 문제가 있다”처럼 단정하는 표현을 절대 사용하지 마세요.
3. 교사가 학생을 긍정적이고 객관적으로 이해하고 대화할 수 있도록 돕는 방향으로 응답하세요.
...
6. 다음 수업에서 해볼 수 있는 작은 지원: 
  ...
  try {
    ...
    if (!resultText) {
      return res.status(500).json({ success: false, error: '응답 결과를 파싱할 수 없습니다.' });
    }
